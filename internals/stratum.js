...
class Counter {
    ...
    next(len = 16) {
        ...
        return hash.slice(0, len).toString('hex');
    }
}
    ...
    handleMessage(miner, message) {
        ...
        switch (message.method) {
            ...
            case 'mining.submit': {
                if (!miner.authorized)
                    return StratumServer.reply(miner, message.id, 'You are not eligible to do such requests', null);
                let [user, jobid, extraNonce2, time, nonce] = message.params;
                let job = this.#templates.find(item => item.jobid == jobid);
                if (!job) {
                    return StratumServer.reply(miner, message.id, 'Job not found', false);
                }
                let coinbase = job.template.serializeCoinbase(miner.extraNonce1, extraNonce2);
                let header = job.template.serializeHeader(coinbase, time, nonce);
